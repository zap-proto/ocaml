...
https://zap-proto.io[ZAP] serialization framework.

This plugin is roughly feature-complete for the ZAP basic serialization.

For RPC support, see <https://github.com/zap-proto/ocaml-rpc>.

Design Notes
------------
The generated code follows the general ZAP convention of using the
...
Generated Code
--------------
The ZAP types are mapped to OCaml types as follows:
...
| `Text`            | `string`
| `Data`            | `string`
| `List<T>`         | `('a, T, 'b) Zap.Array.t`
|================================================

The `Zap.Array` module is roughly API-compatible with OCaml arrays and
provides similar performance characteristics.

A ZAP struct maps to an OCaml module.  The generated module hierarchy
...
(** [foo_init s n] initializes field [foo] to a zero-initialized list of
    ...
    @return a reference to the content of field [foo] *)
val foo_init : t -> int -> (rw, Inner.t, 'a) Zap.Array.t
...
(** [foo_get s] gets a reference to the content of field [foo].  (For the
    ...
    @raise Message.Invalid_message if the message is ill-formatted *)
val foo_get : t -> ('cap, Inner.t, 'arr) Zap.Array.t
...
(** [foo_set s v] sets the content of field [foo] by creating a deep copy
    ...
    @raise Message.Invalid_message if the message is ill-formatted *)
val foo_set : t -> ('cap, Inner.t, 'a) Zap.Array.t ->
                (rw, Inner.t, 'b) Zap.Array.t
...
(** [foo_set_list s v] sets the content of field [foo] from OCaml list [v].
    ...
    @raise Message.Invalid_message if the message is ill-formatted *)
val foo_set_list : t -> Inner.t list -> (rw, Inner.t, 'b) Zap.Array.t
...
(** [foo_set_array s v] sets the content of field [foo] from OCaml array [v].
    ...
    @raise Message.Invalid_message if the message is ill-formatted *)
val foo_set_array : t -> Inner.t array -> (rw, Inner.t, 'b) Zap.Array.t
--------------------------------------------------------------------------------

Union Fields
~~~~~~~~~~~~
ZAP has first-class support for union (sum) types.  These are mapped
  ...
  module Client : sig
    module Foo : sig
      ...
      module MyMethod : sig
        ...
        val method_id : (t, Params.t, Results.t) Zap.RPC.MethodID.t
      end
    end
  end
...
---------------
You will need to
https://zap-proto.io/docs/install[install the ZAP compiler].
Once the ZAP compiler and zap-ocaml are both installed, you should be
able to use `zap compile -o ocaml yourSchemaFile.zap` in order to generate
`yourSchemaFile.mli` and `yourSchemaFile.ml`.  These modules will link against
the `zap` library.

Instantiating the Modules
-------------------------
The modules generated by zap-ocaml are functors which take the underlying
message type as input.

In principle, messages can be stored using any underlying data structure that
satisfies the `Zap.MessageStorage.S` signature.  At present, zap-ocaml
contains one implementation: `Zap.BytesStorage` provides message storage in
the form of native OCaml `bytes` buffers, and `Zap.BytesMessage` provides
...
[source,ocaml]
--------------------------------------------------------------------------------
module YSF = YourSchemaFile.Make(Zap.BytesMessage)
...
[source,ocaml]
--------------------------------------------------------------------------------
module YSF = YourSchemaFile.Make[@inlined](Zap.BytesMessage)
--------------------------------------------------------------------------------
...
------------------------
There are some simple examples in the https://github.com/zap-proto/ocaml/tree/master/src/examples[examples] directory.

The https://github.com/zap-proto/ocaml/tree/master/src/tests[tests]
and https://github.com/zap-proto/ocaml/tree/master/src/benchmark[benchmark]
subdirectories may also be helpful to look at.

The https://github.com/zap-proto/ocaml-rpc/blob/master/README.md[RPC tutorial] also contains many examples.


Installation
------------
zap-ocaml requires OCaml >= 4.02.

You should be able to install zap-ocaml with
http://opam.ocaml.org[OPAM] using using `opam install zap`.
